...
**重要说明**：此处指的是硬分叉升级场景下的数据迁移策略。**日常节点同步时，以太坊原有的节点数据同步逻辑（第 5.2 节描述的同步区块、状态等）仍然正常使用**。硬分叉时本地已有的非秘密数据可以直接复用，无需从其他节点重新同步。
...
- **日常节点同步**：以太坊原有的节点同步逻辑（第 5.2 节描述的区块同步、状态同步等）仍然正常使用
